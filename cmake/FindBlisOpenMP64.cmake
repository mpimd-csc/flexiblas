SET(BLIS_PREFIX "Blis-OpenMP64: ")
SET(BLIS_LIB_NAME OpenMP64)

IF ( EXISTS /etc/debian_version )
    # We are on debian
    MESSAGE(STATUS "${BLIS_PREFIX} ${CMAKE_LIBRARY_ARCHITECTURE} ")
    SET(SEARCH_PATH /usr/lib/${CMAKE_LIBRARY_ARCHITECTURE}/blis64-openmp /usr/lib/ /usr/lib64 /usr/local/lib /usr/local/lib64)
    SET(SEARCH_NAME blis64 blis)
ELSE()
    SET(SEARCH_PATH /usr/lib/ /usr/lib64 /usr/local/lib /usr/local/lib64)
    SET(SEARCH_NAME bliso64 blis64 blis)
ENDIF()
...
IF (Blis${BLIS_LIB_NAME}_LIBRARY)
    ...
    int main() { assert(bli_info_get_int_type_size() == 64 &&  bli_info_get_enable_pthreads() == 0 && bli_info_get_enable_openmp()==1) \; return 0\;}
    ")

    FILE(WRITE ${CMAKE_BINARY_DIR}/btest.c ${TEST_CODE})
    TRY_RUN(_RUN_RES _COMPILE_RES ${CMAKE_BINARY_DIR} ${CMAKE_BINARY_DIR}/btest.c LINK_LIBRARIES ${Blis${BLIS_LIB_NAME}_LIBRARY}
    ...
    ENDIF()
    FILE(REMOVE ${CMAKE_BINARY_DIR}/btest.c)
