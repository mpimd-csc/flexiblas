                 ...
                 double  *C, const int ldc)
{
   ...
   #define F77_ldc ldc
#endif
   current_backend->blas.dsymm.calls[POS_CBLAS] ++;

   if ( current_backend->post_init != 0 ) {
   	__flexiblas_backend_init(current_backend);
   	current_backend->post_init = 0;
   }
   if ( current_backend->blas.dsymm.call_cblas != NULL ) {
	   double te = 0, ts = 0;
	   ...
	   void (*fn)
		 (const CBLAS_LAYOUT layout, const CBLAS_SIDE Side,
                 ...
                 const double  *B, const int ldb, const double beta,
                 double  *C, const int ldc)
		   = current_backend->blas.dsymm.call_cblas;
	fn	(layout,Side,Uplo,M,N,alpha,A,lda,B,ldb,beta,C,ldc);
	if ( __flexiblas_profile ){
	   te = flexiblas_wtime(); 
	   current_backend->blas.dsymm.timings[POS_CBLAS] += (te - ts); 
	}
   } else {
	   ...
	   if( layout == CblasColMajor )
	   {
		 ...
		 F77_SD = C2F_CHAR(&SD);
	      ...
	      FC_GLOBAL(dsymm,DSYMM)(F77_SD, F77_UL, &F77_M, &F77_N, &alpha, A, &F77_lda,
	   ...
	   } else if (layout == CblasRowMajor)
	   {
		 ...
		 F77_SD = C2F_CHAR(&SD);
	      ...
	      FC_GLOBAL(dsymm,DSYMM)(F77_SD, F77_UL, &F77_N, &F77_M, &alpha, A, &F77_lda, B,
