...

{
   ...
   #define F77_ldb ldb
#endif
   current_backend->blas.dtrsm.calls[POS_CBLAS] ++;

   if ( current_backend->post_init != 0 ) {
   	__flexiblas_backend_init(current_backend);
   	current_backend->post_init = 0;
   }
   if ( current_backend->blas.dtrsm.call_cblas != NULL ) {
	   double te = 0, ts = 0;
	   ...
	   void (*fn)
		  (const CBLAS_LAYOUT layout, const CBLAS_SIDE Side,
                 ...
                 const double alpha, const double  *A, const int lda,
                 double  *B, const int ldb)
		   = current_backend->blas.dtrsm.call_cblas;
	fn(layout,Side,Uplo,TransA,Diag,M,N,alpha,A,lda,B,ldb);
        if ( __flexiblas_profile ){
	   te = flexiblas_wtime(); 
	   current_backend->blas.dtrsm.timings[POS_CBLAS] += (te - ts); 
	}
   } else {
	   ...
	   if( layout == CblasColMajor )
	   {
		 ...
		 return;
	      ...
	      FC_GLOBAL(dtrsm,DTRSM)(F77_SD, F77_UL, F77_TA, F77_DI, &F77_M, &F77_N, &alpha,
			A, &F77_lda, B, &F77_ldb);
	   } 
	   else if (layout == CblasRowMajor)
	   {
		 ...
		 return;
	      ...
	      FC_GLOBAL(dtrsm,DTRSM)(F77_SD, F77_UL, F77_TA, F77_DI, &F77_N, &F77_M, &alpha, A, 
		       &F77_lda, B, &F77_ldb);
	   } 
