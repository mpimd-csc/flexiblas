...
static TLS_STORE uint8_t hook_cblas_dspmv_pos = 0;
...
#endif
    if ( current_backend->blas.dspmv.cblas_function != NULL ) {
        void (*fn)
            (const CBLAS_LAYOUT layout,
             const CBLAS_UPLO Uplo, const CBLAS_INT N,
             const double alpha, const double  *AP,
             const double  *X, const CBLAS_INT incX, const double beta,
             double  *Y, const CBLAS_INT incY);
        *(void **) & fn = current_backend->blas.dspmv.cblas_function;
        fn(layout,Uplo,N,alpha,AP,X,incX,beta,Y,incY);
    } else {
        ...
        if (layout == CblasColMajor)
        {
            ...
            F77_UL = C2F_CHAR(&UL);
#endif
            FC_GLOBAL(dspmv,DSPMV)(F77_UL, (blasint *)(uintptr_t)&F77_N, (double *)(uintptr_t)&alpha, (double *)(uintptr_t)AP, (double *)(uintptr_t)X, (blasint *)(uintptr_t)&F77_incX, (double *)(uintptr_t)&beta, Y, (blasint *)(uintptr_t)&F77_incY, 1);
        }
