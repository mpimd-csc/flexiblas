...
#endif
   current_backend->blas.stpsv.calls[POS_CBLAS] ++;

   if ( current_backend->post_init != 0 ) {
   	__flexiblas_backend_init(current_backend);
   	current_backend->post_init = 0;
   }
   if ( current_backend->blas.stpsv.call_cblas != NULL ) {
	   float te = 0, ts = 0;
	   ...
	   void (*fn)
		  (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
                 const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag,
                 const int N, const float  *Ap, float  *X, const int incX)
		   = current_backend->blas.stpsv.call_cblas;
	fn(layout,Uplo,TransA,Diag,N,Ap,X,incX);
	   if ( __flexiblas_profile ){
		   te = flexiblas_wtime(); 
		   current_backend->blas.stpsv.timings[POS_CBLAS] += (te - ts); 
	   }
   } else {
