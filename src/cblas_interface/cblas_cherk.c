    ...
    if ( current_backend->blas.cherk.cblas_function != NULL ) {
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
             ...
             const float alpha, const void *A, const CBLAS_INT lda,
             const float beta, void *C, const CBLAS_INT ldc);
        *(void **) & fn = current_backend->blas.cherk.cblas_function;
        fn(layout,Uplo,Trans,N,K,alpha,A,lda,beta,C,ldc);
    } else {
        ...
        if( layout == CblasColMajor )
        {
            ...
            FC_GLOBAL(cherk,CHERK)(F77_UL, F77_TR, (blasint *)(uintptr_t)&F77_N, (blasint *)(uintptr_t)&F77_K, (void *)(uintptr_t) &alpha, (void *)(uintptr_t) A, (blasint *)(uintptr_t)&F77_lda,
