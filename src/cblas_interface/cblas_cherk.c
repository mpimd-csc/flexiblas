...
#define F77_ldc ldc
    ...
    current_backend->blas.cherk.calls[POS_CBLAS] ++;

    if ( current_backend->post_init != 0 ) {
        __flexiblas_backend_init(current_backend);
        current_backend->post_init = 0;
    }
    if ( current_backend->blas.cherk.call_cblas != NULL ) {
        double te = 0, ts = 0;
        ...
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
             ...
             const float alpha, const void *A, const int lda,
             const float beta, void *C, const int ldc)
            = current_backend->blas.cherk.call_cblas;
        fn(layout,Uplo,Trans,N,K,alpha,A,lda,beta,C,ldc);
        if ( __flexiblas_profile ){
            te = flexiblas_wtime(); 
            current_backend->blas.cherk.timings[POS_CBLAS] += (te - ts); 
        }
    } else {
        ...
        if( layout == CblasColMajor )
        {
            ...
            FC_GLOBAL(cherk,CHERK)(F77_UL, F77_TR, &F77_N, &F77_K, &alpha, A, &F77_lda,
                    &beta, C, &F77_ldc);
