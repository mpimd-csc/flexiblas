                ...
                const int incX, float  *A, const int lda)
{
   ...
   #define F77_lda  lda
#endif
   current_backend->blas.ssyr.calls[POS_CBLAS] ++;

   if ( current_backend->post_init != 0 ) {
   	__flexiblas_backend_init(current_backend);
   	current_backend->post_init = 0;
   }
   if ( current_backend->blas.ssyr.call_cblas != NULL ) {
	   float te = 0, ts = 0;
	   ...
	   void (*fn)
		  (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
                const int N, const float  alpha, const float  *X,
                const int incX, float  *A, const int lda)
		   = current_backend->blas.ssyr.call_cblas;
	fn(layout,Uplo,N,alpha,X,incX,A,lda);
	if ( __flexiblas_profile ){
	   te = flexiblas_wtime(); 
	   current_backend->blas.ssyr.timings[POS_CBLAS] += (te - ts); 
	}
   } else {
	   ...
	   if (layout == CblasColMajor)
	   {
		 ...
		 F77_UL = C2F_CHAR(&UL);
	      ...
	      FC_GLOBAL(ssyr,SSYR)(F77_UL, &F77_N, &alpha, X, &F77_incX, A, &F77_lda);
