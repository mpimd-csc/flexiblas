...
static TLS_STORE uint8_t hook_cblas_strmv_pos = 0;
...
#endif
    if ( current_backend->blas.strmv.cblas_function != NULL ) {
        ...
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
             ...
             const CBLAS_INT N, const float  *A, const CBLAS_INT lda,
             float  *X, const CBLAS_INT incX);
        *(void **) &fn = current_backend->blas.strmv.cblas_function;
        fn(layout,Uplo,TransA,Diag,N,A,lda,X,incX);
    } else {
        ...
        if (layout == CblasColMajor)
        {
            ...
            FC_GLOBAL(strmv,STRMV)( F77_UL, F77_TA, F77_DI, (blasint *)(uintptr_t)&F77_N, (float *)(uintptr_t) A, (blasint *)(uintptr_t)&F77_lda, X,
