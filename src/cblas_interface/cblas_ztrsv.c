...
#endif
   current_backend->blas.ztrsv.calls[POS_CBLAS] ++;

   if ( current_backend->post_init != 0 ) {
   	__flexiblas_backend_init(current_backend);
   	current_backend->post_init = 0;
   }
   if ( current_backend->blas.ztrsv.call_cblas != NULL ) {
	   double te = 0, ts = 0;
	   ...
	   void (*fn)
		  (const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
                 ...
                 const int N, const void  *A, const int lda, void  *X,
                 const int incX)
		   = current_backend->blas.ztrsv.call_cblas;
	fn(order,Uplo,TransA,Diag,N,A,lda,X,incX);
; 
	if ( __flexiblas_profile ){
	   te = flexiblas_wtime(); 
	   current_backend->blas.ztrsv.timings[POS_CBLAS] += (te - ts); 
	}
   } else {
