                       ...
                       const int incX, float *Y, const int incY)
{
   ...
   if ( current_backend->blas.saxpy.call_cblas != NULL ) {
	   float te = 0, ts = 0;
	   if ( __flexiblas_profile ) {
		   ts = flexiblas_wtime(); 
	   }
	   void (*fn)(const int , const float , const float *, const int, const float *Y, const int ) = current_backend->blas.saxpy.call_cblas;
