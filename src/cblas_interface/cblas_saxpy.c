        ...
        const int incX, float *Y, const int incY)
{
    ...
    if ( current_backend->blas.saxpy.cblas_function != NULL ) {
        void (*fn)  ( const int N, const float alpha, const float *X, const int incX, float *Y, const int incY);
        if ( __flexiblas_hooks->saxpy.cblas_hook_function[0] != NULL) {
            fn  = __flexiblas_hooks->saxpy.cblas_hook_function[0];
            hook_cblas_saxpy_pos = 0;
        } else {
...
{
    void (*fn)  ( const int N, const float alpha, const float *X, const int incX, float *Y, const int incY);
    hook_cblas_saxpy_pos ++;
    if ( hook_cblas_saxpy_pos < __flexiblas_hooks->saxpy.cblas_nhook) {
        fn  = __flexiblas_hooks->saxpy.cblas_hook_function[hook_cblas_saxpy_pos];
    } else {
        hook_cblas_saxpy_pos = 0;
        fn = current_backend->blas.saxpy.cblas_function;
    }
