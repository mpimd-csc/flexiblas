...
static TLS_STORE uint8_t hook_cblas_ztpmv_pos = 0;
...
#define F77_incX incX
    ...
    if ( current_backend->blas.ztpmv.cblas_function != NULL ) {
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
             const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag,
             const CBLAS_INT N, const void  *Ap, void  *X, const CBLAS_INT incX);
        *(void **) &fn = current_backend->blas.ztpmv.cblas_function;
        fn(layout,Uplo,TransA,Diag,N,Ap,X,incX);
    } else {
        ...
        if (layout == CblasColMajor)
        {
            ...
            F77_DI = C2F_CHAR(&DI);
#endif
            FC_GLOBAL(ztpmv,ZTPMV)( F77_UL, F77_TA, F77_DI, (blasint *)(uintptr_t)&F77_N, (void *)(uintptr_t) Ap, X, (blasint *)(uintptr_t)&F77_incX, 1, 1, 1);
        }
