...
static TLS_STORE uint8_t hook_cblas_dtpsv_pos = 0;
...
#endif
    if ( current_backend->blas.dtpsv.cblas_function != NULL ) {
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
             const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag,
             const CBLAS_INT N, const double  *Ap, double  *X, const CBLAS_INT incX);
        *(void **) &fn = current_backend->blas.dtpsv.cblas_function;
        ...
        if (layout == CblasColMajor)
        {
            ...
            F77_DI = C2F_CHAR(&DI);
#endif
            FC_GLOBAL(dtpsv,DTPSV)( F77_UL, F77_TA, F77_DI, (blasint *)(uintptr_t)&F77_N, (double *)(uintptr_t)Ap, X, (blasint *)(uintptr_t)&F77_incX, 1,1,1);
        }
