...
static TLS_STORE uint8_t hook_cblas_strsv_pos = 0;
...
#endif
   if ( current_backend->blas.strsv.cblas_function != NULL ) {
	   void (*fn)
		 (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
                 const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag,
                 const CBLAS_INT N, const float  *A, const CBLAS_INT lda, float  *X,
                 const CBLAS_INT incX);
       *(void **) &fn = current_backend->blas.strsv.cblas_function;
   	   fn(layout,Uplo,TransA,Diag,N,A,lda,X,incX);
   } else {
	   ...
	   if (layout == CblasColMajor)
	   {
		 ...
		 F77_DI = C2F_CHAR(&DI);
	      #endif
	      FC_GLOBAL(strsv,STRSV)( F77_UL, F77_TA, F77_DI, (blasint *)(uintptr_t)&F77_N, (float *)(uintptr_t) A, (blasint *)(uintptr_t)&F77_lda, X,
