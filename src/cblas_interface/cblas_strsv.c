...
#endif
   current_backend->blas.strsv.calls[POS_CBLAS] ++;

   if ( current_backend->post_init != 0 ) {
   	__flexiblas_backend_init(current_backend);
   	current_backend->post_init = 0;
   }
   if ( current_backend->blas.strsv.call_cblas != NULL ) {
	   float te = 0, ts = 0;
	   ...
	   void (*fn)
		 (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
                 ...
                 const int N, const float  *A, const int lda, float  *X,
                 const int incX)
		   = current_backend->blas.strsv.call_cblas;
   	   fn(layout,Uplo,TransA,Diag,N,A,lda,X,incX);
	   if ( __flexiblas_profile ){
		   te = flexiblas_wtime(); 
		   current_backend->blas.strsv.timings[POS_CBLAS] += (te - ts); 
	   }
   } else {
	   ...
	   if (layout == CblasColMajor)
	   {
		 ...
		 F77_DI = C2F_CHAR(&DI);
	      #endif
	      FC_GLOBAL(strsv,STRSV)( F77_UL, F77_TA, F77_DI, &F77_N, A, &F77_lda, X,
			      &F77_incX);
	   }
