...
static TLS_STORE uint8_t hook_cblas_cgemmtr_pos = 0;

void cblas_cgemmtr(const CBLAS_LAYOUT layout, const CBLAS_UPLO uplo, const CBLAS_TRANSPOSE TransA,
...
#define F77_ldc ldc
    ...
    if ( current_backend->blas.cgemmtr.cblas_function != NULL ) {
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO uplo, const CBLAS_TRANSPOSE TransA,
             const CBLAS_TRANSPOSE TransB, const CBLAS_INT N,
             const CBLAS_INT K, const void *alpha, const void  *A,
             const CBLAS_INT lda, const void  *B, const CBLAS_INT ldb,
             const void *beta, void  *C, const CBLAS_INT ldc);
        *(void **) & fn = current_backend->blas.cgemmtr.cblas_function;
        fn(layout,uplo,TransA,TransB,N,K,alpha,A,lda,B,ldb,beta,C,ldc);
    } else {
        ...
        if( layout == CblasColMajor )
        {
            if ( uplo == CblasUpper ) UL = 'U';
                ...
                cblas_xerbla(2, "cblas_cgemmtr", "Illegal Uplo setting, %d\n", uplo);
                CBLAS_CallFromC = 0;
                RowMajorStrg = 0;
                return;
            }
            ...
            {
                cblas_xerbla(3, "cblas_cgemmtr", "Illegal TransA setting, %d\n", TransA);
                CBLAS_CallFromC = 0;
                RowMajorStrg = 0;
                return;
            }
            ...
            {
                cblas_xerbla(4, "cblas_cgemmtr", "Illegal TransB setting, %d\n", TransB);
                CBLAS_CallFromC = 0;
                RowMajorStrg = 0;
                return;
            }
            ...
            FC_GLOBAL(cgemmtr,CGEMMTR)(F77_UL, F77_TA, F77_TB, (blasint *)(uintptr_t)&F77_N, (blasint *)(uintptr_t)&F77_K, (void *)(uintptr_t) alpha, (void *)(uintptr_t) A,
        ...
        } else if (layout == CblasRowMajor)
        {
            ...
            FC_GLOBAL(cgemmtr,CGEMMTR)(F77_UL, F77_TA, F77_TB, (blasint *)(uintptr_t)&F77_N, (blasint *)(uintptr_t)&F77_K, (void *)(uintptr_t) alpha, (void *)(uintptr_t) B,
        ...
        else cblas_xerbla(1, "cblas_cgemmtr", "Illegal layout setting, %d\n", layout);
        CBLAS_CallFromC = 0;
        RowMajorStrg = 0;
        return;
