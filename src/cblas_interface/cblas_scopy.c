...
void cblas_scopy( const int N, const float *X,const int incX, float *Y, const int incY)
{
    ...
    if ( current_backend->blas.scopy.cblas_function != NULL ) {
        float (*fn)  ( const int N, const float *X, const int incX, float *Y, const int incY);

        if ( __flexiblas_hooks->scopy.cblas_hook_function[0] != NULL) {
            fn  = __flexiblas_hooks->scopy.cblas_hook_function[0];
            hook_cblas_scopy_pos = 0;
        } else {
...
void flexiblas_chain_cblas_scopy( const int N, const float *X,const int incX, float *Y, const int incY)
{
    ...
    float (*fn)  ( const int N, const float *X, const int incX, float *Y, const int incY);
    hook_cblas_scopy_pos ++;
    if ( hook_cblas_scopy_pos < __flexiblas_hooks->scopy.cblas_nhook) {
        fn  = __flexiblas_hooks->scopy.cblas_hook_function[hook_cblas_scopy_pos];
    } else {
        hook_cblas_scopy_pos = 0;
        fn = current_backend->blas.scopy.cblas_function;
    }
