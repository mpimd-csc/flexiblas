...
static TLS_STORE uint8_t hook_cblas_ssymv_pos = 0;
...
#define F77_incY incY
    ...
    if ( current_backend->blas.ssymv.cblas_function != NULL ) {
        void (*fn)
            (const CBLAS_LAYOUT layout,
             const CBLAS_UPLO Uplo, const CBLAS_INT N,
             const float alpha, const float  *A, const CBLAS_INT lda,
             const float  *X, const CBLAS_INT incX, const float beta,
             float  *Y, const CBLAS_INT incY);
        *(void **) & fn = current_backend->blas.ssymv.cblas_function;
        fn(layout,Uplo,N,alpha,A,lda,X,incX,beta,Y,incY);
    } else {
        ...
        if (layout == CblasColMajor)
        {
            ...
            F77_UL = C2F_CHAR(&UL);
#endif
            FC_GLOBAL(ssymv,SSYMV)(F77_UL, (blasint *)(uintptr_t)&F77_N, (float *)(uintptr_t) &alpha, (float *)(uintptr_t) A, (blasint *)(uintptr_t)&F77_lda, (float *)(uintptr_t) X, (blasint *)(uintptr_t)&F77_incX, (float *)(uintptr_t) &beta, Y, (blasint *)(uintptr_t)&F77_incY, 1);
        }
