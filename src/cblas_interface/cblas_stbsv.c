...
static TLS_STORE uint8_t hook_cblas_stbsv_pos = 0;
...
#endif
    if ( current_backend->blas.stbsv.cblas_function != NULL ) {
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
             const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag,
             const CBLAS_INT N, const CBLAS_INT K, const float  *A, const CBLAS_INT lda,
             float  *X, const CBLAS_INT incX);
        *(void **) &fn = current_backend->blas.stbsv.cblas_function;
        fn(layout,Uplo,TransA,Diag,N,K,A,lda,X,incX);
    } else {
        ...
        if (layout == CblasColMajor)
        {
            ...
            F77_DI = C2F_CHAR(&DI);
#endif
            FC_GLOBAL(stbsv,STBSV)( F77_UL, F77_TA, F77_DI, (blasint *)(uintptr_t)&F77_N, (blasint *)(uintptr_t)&F77_K, (float *)(uintptr_t) A, (blasint *)(uintptr_t)&F77_lda, X, (blasint *)(uintptr_t)&F77_incX, 1, 1, 1);
        }
