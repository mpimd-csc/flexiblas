...
void cblas_zcopy( const int N, const void *X,const int incX, void *Y, const int incY)
{
    ...
    if ( current_backend->blas.zcopy.cblas_function != NULL ) {
        void (*fn)  ( const int N, const void *X, const int incX, void *Y, const int incY);

        if ( __flexiblas_hooks->zcopy.cblas_hook_function[0] != NULL) {
            fn  = __flexiblas_hooks->zcopy.cblas_hook_function[0];
            hook_cblas_zcopy_pos = 0;
        } else {
            fn = current_backend->blas.zcopy.cblas_function;
        }
...
void flexiblas_chain_cblas_zcopy( const int N, const void *X,const int incX, void *Y, const int incY)
{
    ...
    void (*fn)  ( const int N, const void *X, const int incX, void *Y, const int incY);
    hook_cblas_zcopy_pos ++;
    if ( hook_cblas_zcopy_pos < __flexiblas_hooks->zcopy.cblas_nhook) {
        fn  = __flexiblas_hooks->zcopy.cblas_hook_function[hook_cblas_zcopy_pos];
    } else {
        hook_cblas_zcopy_pos = 0;
        fn = current_backend->blas.zcopy.cblas_function;
    }
