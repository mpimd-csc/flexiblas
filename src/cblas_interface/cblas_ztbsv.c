...
static TLS_STORE uint8_t hook_cblas_ztbsv_pos = 0;
...
#define F77_incX incX
    ...
    if ( current_backend->blas.ztbsv.cblas_function != NULL ) {
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
             const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag,
             const CBLAS_INT N, const CBLAS_INT K, const void  *A, const CBLAS_INT lda,
             void  *X, const CBLAS_INT incX);
        *(void **) &fn = current_backend->blas.ztbsv.cblas_function;
        fn(layout,Uplo,TransA,Diag,N,K,A,lda,X,incX);
    } else {
        ...
        if (layout == CblasColMajor)
        {
            ...
            F77_DI = C2F_CHAR(&DI);
#endif
            FC_GLOBAL(ztbsv,ZTBSV)( F77_UL, F77_TA, F77_DI, (blasint *)(uintptr_t)&F77_N, (blasint *)(uintptr_t)&F77_K, (void *)(uintptr_t) A, (blasint *)(uintptr_t)&F77_lda, X,
