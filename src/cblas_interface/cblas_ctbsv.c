...
#endif
    current_backend->blas.ctbsv.calls[POS_CBLAS] ++;

    if ( current_backend->post_init != 0 ) {
        __flexiblas_backend_init(current_backend);
        current_backend->post_init = 0;
    }
    if ( current_backend->blas.ctbsv.call_cblas != NULL ) {
        double te = 0, ts = 0;
        ...
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
             ...
             const int N, const int K, const void  *A, const int lda,
             void  *X, const int incX)
            = current_backend->blas.ctbsv.call_cblas;
        fn(layout,Uplo,TransA,Diag,N,K,A,lda,X,incX);
        if ( __flexiblas_profile ){
            te = flexiblas_wtime(); 
            current_backend->blas.ctbsv.timings[POS_CBLAS] += (te - ts); 
        }
    } else {
        int n, i=0, tincX;
