...
static TLS_STORE uint8_t hook_cblas_izamax_pos = 0;
...
CBLAS_INDEX cblas_izamax( const CBLAS_INT N, const void *X, const CBLAS_INT incX)
{
    ...
    CBLAS_BACKEND_INIT();
    CBLAS_HOOK_SELECT(izamax);
    iamax = fn(N,X,incX);

    return iamax;
}

CBLAS_INDEX flexiblas_chain_cblas_izamax( const CBLAS_INT N, const void *X, const CBLAS_INT incX)
{
    CBLAS_INT iamax;

    CBLAS_INDEX (*fn) ( const CBLAS_INT N, const void *X, const CBLAS_INT incX);
    CBLAS_HOOK_ADVANCE(izamax);
    iamax = fn(N,X,incX);
    return iamax;

}

CBLAS_INDEX flexiblas_real_cblas_izamax( const CBLAS_INT N, const void *X, const CBLAS_INT incX)
{
    CBLAS_INT iamax;
#ifdef F77_INT
