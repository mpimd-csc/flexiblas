...
double cblas_ddot( const int N, const double *X, const int incX, const double *Y, const int incY)
{
    ...
    if ( current_backend->blas.ddot.cblas_function != NULL ) {
        ...
        if ( __flexiblas_hooks->ddot.cblas_hook_function[0] != NULL) {
            fn  = __flexiblas_hooks->ddot.cblas_hook_function[0];
            hook_cblas_ddot_pos = 0;
        } else {
        ...
        d = FC_GLOBAL(ddot,DDOT)( &F77_N, X, &F77_incX, Y, &F77_incY);
    }
    return d;
}
...
double flexiblas_chain_cblas_ddot( const int N, const double *X, const int incX, const double *Y, const int incY)
{
    double (*fn)  ( const int N, const double *X, const int incX, const double *Y, const int incY);
    hook_cblas_ddot_pos ++;
    if ( hook_cblas_ddot_pos < __flexiblas_hooks->ddot.cblas_nhook) {
        fn  = __flexiblas_hooks->ddot.cblas_hook_function[hook_cblas_ddot_pos];
    } else {
        hook_cblas_ddot_pos = 0;
        fn = current_backend->blas.ddot.cblas_function;
    }
