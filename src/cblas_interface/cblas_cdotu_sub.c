...
void cblas_cdotu_sub( const int N, const void *X, const int incX, const void *Y, const int incY,void *dotc)
{
    ...
    if ( current_backend->blas.cdotu_sub.cblas_function != NULL ) {
        ...
        if ( __flexiblas_hooks->cdotu_sub.cblas_hook_function[0] != NULL) {
            fn  = __flexiblas_hooks->cdotu_sub.cblas_hook_function[0];
            hook_cblas_cdotu_sub_pos = 0;
        } else {
...
{
    void (*fn)  ( const int N, const void *X, const int incX, const void *Y, const int incY,void *dotc);
    hook_cblas_cdotu_sub_pos ++;
    if ( hook_cblas_cdotu_sub_pos < __flexiblas_hooks->cdotu_sub.cblas_nhook) {
        fn  = __flexiblas_hooks->cdotu_sub.cblas_hook_function[hook_cblas_cdotu_sub_pos];
    } else {
        hook_cblas_cdotu_sub_pos = 0;
        fn = current_backend->blas.cdotu_sub.cblas_function;
    }
