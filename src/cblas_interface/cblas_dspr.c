...
#endif
   current_backend->blas.dspr.calls[POS_CBLAS] ++;

   if ( current_backend->post_init != 0 ) {
   	__flexiblas_backend_init(current_backend);
   	current_backend->post_init = 0;
   }
   if ( current_backend->blas.dspr.call_cblas != NULL ) {
	   double te = 0, ts = 0;
	   ...
	   void (*fn)
		(const enum CBLAS_ORDER order, const enum CBLAS_UPLO Uplo,
                const int N, const double alpha, const double *X,
                const int incX, double *Ap)
		   = current_backend->blas.dspr.call_cblas;
	fn(order,Uplo,N,alpha,X,incX,Ap);
	if ( __flexiblas_profile ){
	   te = flexiblas_wtime(); 
	   current_backend->blas.dspr.timings[POS_CBLAS] += (te - ts); 
	}
   } else {
