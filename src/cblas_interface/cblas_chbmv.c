...
#endif
    current_backend->blas.chbmv.calls[POS_CBLAS] ++;

    if ( current_backend->post_init != 0 ) {
        __flexiblas_backend_init(current_backend);
        current_backend->post_init = 0;
    }
    if ( current_backend->blas.chbmv.call_cblas != NULL ) {
        double te = 0 , ts = 0;
        ...
        void (*fn)
            (const CBLAS_LAYOUT layout,
             ...
             const void  *X, const int incX, const void *beta,
             void  *Y, const int incY)
            = current_backend->blas.chbmv.call_cblas;
        fn(layout,Uplo,N,K,alpha,A,lda,X,incX,beta,Y,incY);
        if ( __flexiblas_profile ) {
            te = flexiblas_wtime(); 
            current_backend->blas.chbmv.timings[POS_CBLAS] += (te - ts); 
        }
    } else {
        int n, i=0, incx=incX;
        ...
        if (layout == CblasColMajor)
        {
            ...
            F77_UL = C2F_CHAR(&UL);
#endif
            FC_GLOBAL(chbmv,CHBMV)(F77_UL, &F77_N, &F77_K, alpha, A, &F77_lda, X,
        ...
        else if (layout == CblasRowMajor)
        {
            ...
            F77_UL = C2F_CHAR(&UL);
#endif
            FC_GLOBAL(chbmv,CHBMV)(F77_UL, &F77_N, &F77_K, ALPHA,
