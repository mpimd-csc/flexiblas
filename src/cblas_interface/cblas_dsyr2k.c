                  ...
                  double  *C, const int ldc)
{
   ...
   #define F77_ldc ldc
#endif
   current_backend->blas.dsyr2k.calls[POS_CBLAS] ++;

   if ( current_backend->post_init != 0 ) {
   	__flexiblas_backend_init(current_backend);
   	current_backend->post_init = 0;
   }
   if ( current_backend->blas.dsyr2k.call_cblas != NULL ) {
	   double te = 0, ts = 0;
	   ...
	   void (*fn)
		  (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
                  ...
                  const double  *B, const int ldb, const double beta,
                  double  *C, const int ldc)
		   = current_backend->blas.dsyr2k.call_cblas;
	fn(layout,Uplo,Trans,N,K,alpha,A,lda,B,ldb,beta,C,ldc);
        if ( __flexiblas_profile ){
	   te = flexiblas_wtime(); 
	   current_backend->blas.dsyr2k.timings[POS_CBLAS] += (te - ts); 
	}
   } else {
	   ...
	   if( layout == CblasColMajor )
	   {
		 ...
		 F77_TR = C2F_CHAR(&TR);
	      ...
	      FC_GLOBAL(dsyr2k,DSYR2K)(F77_UL, F77_TR, &F77_N, &F77_K, &alpha, A, &F77_lda,
	   ...
	   } else if (layout == CblasRowMajor)
	   {
		 ...
		 F77_TR = C2F_CHAR(&TR);
	      ...
	      FC_GLOBAL(dsyr2k,DSYR2K)(F77_UL, F77_TR, &F77_N, &F77_K, &alpha, A, &F77_lda, B, 
