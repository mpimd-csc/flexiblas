...
static TLS_STORE uint8_t hook_cblas_dtbmv_pos = 0;
...
#endif
    if ( current_backend->blas.dtbmv.cblas_function != NULL ) {
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
             const CBLAS_TRANSPOSE TransA, const CBLAS_DIAG Diag,
             const CBLAS_INT N, const CBLAS_INT K, const double  *A, const CBLAS_INT lda,
             double  *X, const CBLAS_INT incX);
        *(void **) &fn = current_backend->blas.dtbmv.cblas_function;
        fn(layout,Uplo,TransA,Diag,N,K,A,lda,X,incX);
    } else {
        ...
        if (layout == CblasColMajor)
        {
            ...
            F77_DI = C2F_CHAR(&DI);
#endif
            FC_GLOBAL(dtbmv,DTBMV)( F77_UL, F77_TA, F77_DI, (blasint *)(uintptr_t)&F77_N, (blasint *)(uintptr_t)&F77_K, (double *)(uintptr_t)A, (blasint *)(uintptr_t)&F77_lda, X,
