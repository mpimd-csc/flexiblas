...
#endif
    current_backend->blas.chpr2.calls[POS_CBLAS] ++;

    if ( current_backend->post_init != 0 ) {
        __flexiblas_backend_init(current_backend);
        current_backend->post_init = 0;
    }
    if ( current_backend->blas.chpr2.call_cblas != NULL ) {
        double ts = 0, te = 0;
        ...
        void (*fn)
            (const CBLAS_LAYOUT layout, const CBLAS_UPLO Uplo,
             const int N,const void *alpha, const void *X, 
             const int incX,const void *Y, const int incY, void *Ap)
            = current_backend->blas.chpr2.call_cblas;
        fn(layout,Uplo,N,alpha,X,incX,Y,incY,Ap);
        if ( __flexiblas_profile ){
            te = flexiblas_wtime(); 
            current_backend->blas.chpr2.timings[POS_CBLAS] += (te - ts); 
        }
    } else {
