...
{
    void (*fn)  ( const int N, const void *X, const int incX, const void *Y, const int incY,void *dotc);
    hook_cblas_cdotc_sub_pos ++;
    if ( hook_cblas_cdotc_sub_pos < __flexiblas_hooks->cdotc_sub.cblas_nhook) {
        fn  = __flexiblas_hooks->cdotc_sub.cblas_hook_function[hook_cblas_cdotc_sub_pos];
    } else {
        hook_cblas_cdotc_sub_pos = 0;
        fn = current_backend->blas.cdotc_sub.cblas_function;
    }
