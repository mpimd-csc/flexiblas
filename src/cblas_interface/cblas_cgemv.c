...
#define F77_incY incY
    ...
    current_backend->blas.cgemv.calls[POS_CBLAS] ++;

    if ( current_backend->post_init != 0 ) {
        __flexiblas_backend_init(current_backend);
        current_backend->post_init = 0;
    }
    if ( current_backend->blas.cgemv.call_cblas != NULL ) {
        double te = 0 , ts = 0;
        ...
        void (*fn)
            (const CBLAS_LAYOUT layout,
             ...
             const void  *X, const int incX, const void *beta,
             void  *Y, const int incY)
            = current_backend->blas.cgemv.call_cblas;
        fn(layout,TransA,M,N,alpha,A,lda,X,incX,beta,Y,incY);
        if ( __flexiblas_profile ) {
            te = flexiblas_wtime(); 
            current_backend->blas.cgemv.timings[POS_CBLAS] += (te - ts); 
        }
    } else {
