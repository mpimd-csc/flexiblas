...
void cblas_ccopy( const int N, const void *X,const int incX, void *Y, const int incY)
{
    ...
    if ( current_backend->blas.ccopy.cblas_function != NULL ) {
        ...
        if ( __flexiblas_hooks->ccopy.cblas_hook_function[0] != NULL) {
            fn  = __flexiblas_hooks->ccopy.cblas_hook_function[0];
            hook_cblas_ccopy_pos = 0;
        } else {
            fn = current_backend->blas.ccopy.cblas_function;
        }
...
void flexiblas_chain_cblas_ccopy( const int N, const void *X,const int incX, void *Y, const int incY)
{
    ...
    void (*fn)  ( const int N, const void *X, const int incX, void *Y, const int incY);
    hook_cblas_ccopy_pos ++;
    if ( hook_cblas_ccopy_pos < __flexiblas_hooks->ccopy.cblas_nhook) {
        fn  = __flexiblas_hooks->ccopy.cblas_hook_function[hook_cblas_ccopy_pos];
    } else {
        hook_cblas_ccopy_pos = 0;
        fn = current_backend->blas.ccopy.cblas_function;
    }
