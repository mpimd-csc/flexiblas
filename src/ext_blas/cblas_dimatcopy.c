		     ...
		     const int cldb)
{
   ...
   if ( current_backend->extblas.dimatcopy.call_cblas != NULL ) {
	   float te = 0, ts = 0;
	   if ( __flexiblas_profile ) {
		   ts = flexiblas_wtime(); 
	   }
	   void (*fn)(const enum CBLAS_ORDER, const enum CBLAS_TRANSPOSE, const int, const int, const double, double *, const int, const int) = current_backend->extblas.dimatcopy.call_cblas;
	   fn(CORDER, CTRANS, crows, ccols, calpha, a, clda, cldb);  
	   if ( __flexiblas_profile ){
		   te = flexiblas_wtime(); 
		   current_backend->extblas.dimatcopy.timings[POS_CBLAS] += (te - ts); 
	   }
   } else {
	char ORDER[2]=" ";
	char TRANS[2]=" "; 
   ...
   current_backend->extblas.dimatcopy.calls[POS_CBLAS] ++;
} 
