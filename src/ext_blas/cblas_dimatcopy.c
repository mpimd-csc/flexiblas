		     ...
		     const int cldb)
{
   ...
   if ( current_backend->blas.dimatcopy.call_cblas != NULL ) {
	   void (*fn)(const CBLAS_ORDER, const CBLAS_TRANSPOSE, const int, const int, const double, double *, const int, const int) = current_backend->blas.dimatcopy.call_cblas;
   ...
   current_backend->blas.dimatcopy.calls[POS_CBLAS] ++;
}
