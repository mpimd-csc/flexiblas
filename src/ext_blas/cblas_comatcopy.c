		     ...
		     void *b, const int cldb)
{
   ...
   if ( current_backend->blas.comatcopy.call_cblas != NULL ) {
	   void (*fn)(const CBLAS_ORDER, const CBLAS_TRANSPOSE, const int, const int, const void *, const void*, const int, void *, const int) = current_backend->blas.comatcopy.call_cblas;
   ...
   current_backend->blas.comatcopy.calls[POS_CBLAS] ++;
}
