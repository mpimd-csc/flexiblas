		     ...
		     const int cldb)
{
   ...
   if ( current_backend->blas.zimatcopy.call_cblas != NULL ) {
	   void (*fn)(const CBLAS_ORDER, const CBLAS_TRANSPOSE, const int, const int, const void *, void*,const int, const int) = current_backend->blas.zimatcopy.call_cblas;
   ...
   current_backend->blas.zimatcopy.calls[POS_CBLAS] ++;
}
