		     ...
		     void *b, const int cldb)
{
   ...
   if ( current_backend->blas.zomatcopy.call_cblas != NULL ) {
	   void (*fn)(const CBLAS_ORDER, const CBLAS_TRANSPOSE, const int, const int, const void *, const void*, const int, void *, const int) = current_backend->blas.zomatcopy.call_cblas;
   ...
   current_backend->blas.zomatcopy.calls[POS_CBLAS] ++;
}
