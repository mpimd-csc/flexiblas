		     ...
		     float *b, const int cldb)
{
   ...
   if ( current_backend->blas.somatcopy.call_cblas != NULL ) {
	   void (*fn)(const CBLAS_ORDER, const CBLAS_TRANSPOSE, const int, const int, const float, const float *, const int, float *, const int) = current_backend->blas.somatcopy.call_cblas;
