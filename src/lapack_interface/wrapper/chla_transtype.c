...
#endif 
{
	...
	fn = current_backend->lapack.chla_transtype.call_fblas; 
	if ( __flexiblas_profile ) {
		ts = flexiblas_wtime(); 
		ret = fn((void*) trans); 
		current_backend->lapack.chla_transtype.timings[0] += (flexiblas_wtime() -ts);
		current_backend->lapack.chla_transtype.calls[0]++;
	} else { 
		ret = fn((void*) trans); 
	} 
	return ret; 
